defmodule Steemex.Ops.TransformersTest do
  ...
  test "transfer op cleaned correctly " do
    ...
    prepared = Ops.Munger.parse(op)
    assert prepared == %MungedOps.Transfer{to: "bob", from: "alice", amount: 100.0, token: "SBD", memo: "nice cypher you've got there"}
  end
  ...
  test "comment op cleaned correctly " do
    ...
    assert prepared == %MungedOps.Comment{body: "body1", title: nil, author: "alice", permlink: "permlink1", tags: json_metadata.tags, app: json_metadata.app, json_metadata: json_metadata, parent_author: "parent_author1", parent_permlink: "parent_permlink1"}
  end
  ...
  test "comment with no parent_author op cleaned correctly " do
    ...
    prepared = Ops.Munger.parse(op)
    json_metadata = %{tags: ["tag1"], app: "steemit/0.1"}
    assert prepared == %MungedOps.Comment{body: "body1", title: nil, author: "alice", permlink: "permlink1", tags: json_metadata.tags, app: json_metadata.app, json_metadata: json_metadata, parent_author: nil, parent_permlink: "category"}
  end
  ...
  test "follow op cleaned and parsed correctly " do
    ...
    prepared = Ops.Munger.parse(op)
    assert prepared == %MungedOps.Follow{follower: "follower1", following: "following1", what: ["blog"]}
  end
  ...
  test "reblog op cleaned and parsed correctly " do
    ...
    prepared = Ops.Munger.parse(op)
    assert prepared == %MungedOps.Reblog{account: "account1", author: "author1", permlink: "permlink1"}
  end
  ...
  test "transfer_to_vesting op cleaned and parsed correctly " do
    ...
    prepared = Ops.Munger.parse(op)
    assert prepared == %MungedOps.TransferToVesting{to: "account1", from: "account2", amount: 3.14, token: "STEEM"}
  end
  ...
  test "feed_publish op cleaned and parsed correctly " do
    ...
    prepared = Ops.Munger.parse(op)
    assert prepared == %MungedOps.FeedPublish{publisher: "account1", base_token: "SBD", base_amount: 2.295, quote_token: "STEEM", quote_amount: 1.0}
  end
